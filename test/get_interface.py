...
def _get_ip(interface: str) -> str:
    try:
        ...
        ipaddr = fcntl.ioctl(sock.fileno(), 0x8927,  struct.pack('256s', interface.encode()))[20:24]
...
def _get_mac(interface: str) -> str:
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        macaddr = fcntl.ioctl(sock.fileno(), 0x8915, struct.pack('256s', interface.encode()))[18:24]
